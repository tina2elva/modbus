...
func (ms *ModbusServer) Start() (err error) {
	...
	case modbusRTU:
		...
		ms.transport = newRTUTransport(
			spw, ms.conf.URL, ms.conf.Speed, ms.conf.Timeout, ms.conf.Logger)

		go ms.acceptSerialClients()
...
func (ms *ModbusServer) Stop() (err error) {
	...
	} else if ms.transportType == modbusRTU {
		ms.transport.Close()
	}
...
func (ms *ModbusServer) acceptSerialClients() {
	ms.handleTransport(
		ms.transport,
		"", "")
}
